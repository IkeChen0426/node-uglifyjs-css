function productCountdown(productName, startTime, endTime) {
    ...
    function changeTime() {
        ...
        if (dif2 > 0) {
            ...
            $("." + productName + "-time-title").text("拼团活动开始倒计时");
            showTime = dif2;
        } else if (dif1 > 0) {
            $("." + productName + "-snap-up").addClass("snap-up-on").text("立即拼团");
            $("." + productName + "-time-title").text("拼团活动结束倒计时");
            showTime = dif1;
        } else {
            $("." + productName + "-snap-up").removeClass("snap-up-on").text("已结束");
            $("." + productName + "-time-title").text("拼团活动已结束");
            showTime = 0;
            clearInterval(Timer);
        }
