$(function () {
    ...
    */
    // 服务器和本地时间差
    var localServiceDiff = new Date(server.currentDate.replace(/-/g, '/')).getTime() - new Date().getTime();
    ...
    function timer() {
        setInterval(function () {
            var timers = calculateDiffTime(V.avtivityEndTime);
            
            if (timers.timediff > 0) {
                if (timers.day > 0) {
                    $(".fooww-activity-day").text(timers.day + '天').show();
                } else {
                    $(".fooww-activity-day").hide();
                }
                $(".fooww-activity-hour").text(timers.hour);
                $(".fooww-activity-minute").text(timers.minute);
                $(".fooww-activity-second").text(timers.second);
                //场景1：未开始
            } 
    ...
    function appointmentShowOrNot() {
        ...
        var hiddenHeight = document.getElementsByClassName("m-reservation")[0].offsetTop -100
...
$(function () {
    ...
    setInterval(function () {
        ...
        if (Math.abs(curMarginTop) >= userWrapH - 60) {
            ...
            $(".barrage-wrap ul").animate({ "margin-top": curMarginTop - 40 });
        }
    }, 4000);
    /*
    var num = 0;
    ...
    function setNotice() {
        ...
        if (V.notice) {
            ...
            noticeWidth = parseInt($(".anim-notice").css("width")) + spaceWidth;
    ...
    function getContent(i) {
        ...
        barrageInfo = newUserList[i][0] + newUserList[i][1] + 's前' + barrageInfo;
